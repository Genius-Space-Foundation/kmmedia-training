...
export function Navbar({ onApplyClick }: { onApplyClick?: () => void }) {
  ...
  const applyScrolledStyle = isScrolled || pathname !== "/";
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-10 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-3 group">
          ...
          <span className={`text-xl font-bold transition-colors ${
            applyScrolledStyle ? "text-brand-text-primary dark:text-white" : "text-white drop-shadow-md"
          }`}>
        ...
        <div className="hidden md:flex items-center gap-10">
            ...
            {navLinks.map((link) => (
              <Link
                ...
                className={`text-sm font-medium transition-colors hover:text-brand-primary ${
                    applyScrolledStyle ? "text-brand-text-secondary dark:text-neutral-300" : "text-white/90 drop-shadow-sm hover:text-white"
                }`}
              >
                {link.name}
              </Link>
          ...
          <ThemeToggle isScrolled={applyScrolledStyle} />
          {onApplyClick ? (
            <button
            ...
            <Link
              href="/#programmes"
              className="px-6 py-2 bg-brand-primary hover:bg-brand-secondary text-white rounded-full text-sm font-bold transition-all hover:shadow-lg hover:shadow-brand-primary/25 active:scale-95 text-center flex items-center"
            >
              Apply Now
            </Link>
          )}
        ...
        <div className="md:hidden flex items-center gap-2">
          <ThemeToggle isScrolled={applyScrolledStyle} />
          <button
            className={`p-2 transition-colors ${applyScrolledStyle ? "text-brand-text-primary dark:text-white" : "text-white"}`}
            ...
            <div className="flex flex-col p-4 gap-4">
              ...
              {onApplyClick ? (
                ...
                <Link
                  href="/#programmes"
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="w-full text-center py-4 bg-brand-primary text-white rounded-xl font-bold shadow-lg shadow-brand-primary/20 block"
                >
                  Apply Now
                </Link>
              )}
