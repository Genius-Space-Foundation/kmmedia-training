...
export async function generateMetadata({ params }: PageProps) {
  ...
  const event = upcomingEvents.find(e => e.id === resolvedParams.id);
  if (!event) {
    return { title: "Event Not Found" };
  }
  return {
    title: `${event.title} | KM Media Events`,
        ...
        <article className="max-w-4xl mx-auto px-4 md:px-10">
          ...
          <div className="space-y-6 mb-10">
            <div className="flex flex-wrap items-center gap-4">
              <span className="px-3 py-1 bg-brand-secondary/10 text-brand-secondary text-sm font-bold rounded-full">
                Upcoming Event
              </span>
